...
- `curl`
 ...
 - `vim`
    - The vim package provides a command line tool, `xxd`. It is used to encode non-UTF-8 characters to valid UTF-8 format (example: convert `Ã` to `%c3%83`
- `jq`
   - The jq package is used to process json responses received through the Telegram API.
- `bash`
...
The `bash`, `curl`, `vim` and `jq` packages are indispensable. Unalix cannot run without them.
...
```
# apt -y update
# apt -y install 'curl' 'jq' 'vim' 'idn' 'idn2'
```
...
- **ShellBot**
  ...
  - License: [GNU General Public License v0.3](http://github.com/shellscriptx/shellbot/blob/master/LICENSE.txt)
...
- **ClearURLs**
  ...
  - License: [GNU Lesser General Public License v0.3](http://gitlab.com/KevinRoebert/ClearUrls/blob/master/LICENSE)
